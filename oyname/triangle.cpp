...
int main_off3()
{
    ...
    camera->transform.Position(15.0f, 0.0f, -5.0f);
    camera->transform.Rotate(-10.0f, -60.0f, 0.0f);

    mesh->transform.Rotate(0.0f, 30.0f, 0.0f);
    mesh->transform.Position(-5.0f, 5.0f, 0.0f);

    mesh2->transform.Rotate(0.0f, 0.0f, 0.0f);
    mesh2->transform.Position(5.0f, 0.0f, 5.0f);

    Engine::PositionEntity(light, -15.0f, 10.0f, 0.0f);
    Engine::PositionEntity(lightMesh, -15.0f, 10.0f, 0.0f);

    lightMesh->transform.Scale(0.5f, 0.1f, 0.5f);
    g_mesh3->transform.Scale(1.0f, 1.0f, 1.0f);
    g_mesh3->transform.Position(0.0f, 0.0f, -5.0f);
    ...
    while (gdx::MainLoop() && !(GetAsyncKeyState(VK_ESCAPE) & 0x8000))
    {
        ...
        //light->transform.Move(0.01f, -0.00f, 0.0f);
        //lightMesh->transform.Move(0.01f, -0.00f, 0.0f);

        mesh->transform.Turn(1.0f, 0.0f, 1.0f);
        mesh2->transform.Turn(0.0f, 0.0f, 0.5f);
...
void MoveObjectInCircle(float centerX, float centerZ, float radius, float angle) {
    ...
    g_mesh3->transform.Turn(0.0f, -0.68f, 0.0f);

    // Setzen der neuen Position des Objekts
    g_mesh3->transform.Position(newX, 0.0f, newZ);
}
